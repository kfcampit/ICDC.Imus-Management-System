class AppointmentObject{
  ...
  String unixToDate(int timestamp){
    ...
    String datetime = tsdate.month.toString() + "-" + tsdate.day.toString() + "-" + tsdate.year.toString();
    return datetime;
  }
  ...
  void timeTo24Hour(String iTime){
    ...
    if(typeOfDay == "PM"){
      try{
        if(iTime.substring(0,2) == "12") time = iTime.substring(0,5);
        else time = (int.parse(iTime.substring(0,2))+12).toString() + iTime.substring(2,5);
      }
      catch(e){
        time = (int.parse(iTime.substring(0,1))+12).toString() + iTime.substring(1,4);
      }
    }
    else{
      if(iTime.length == 7) time = "0" + iTime.substring(0,4);
      else time = iTime.substring(0,5);
    }
  }

  void timeTo12Hour(String iTime){
    if(iTime.substring(0,2) == "00") time = "12" + iTime.substring(2) + " AM";
    else if(int.parse(iTime.substring(0,2)) < 12) time = iTime + " AM";
    else if(int.parse(iTime.substring(0,2)) == 12) time = iTime + " PM";
    else if((int.parse(iTime.substring(0,2))-12).toString().length < 2) time = "0" + (int.parse(iTime.substring(0,2))-12).toString() + iTime.substring(2) + " PM";
    else time = (int.parse(iTime.substring(0,2))-12).toString() + iTime.substring(2) + " PM";
  }
}
